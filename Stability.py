...
def main():
    ...
    N_values = [1, 2, 3, 500, 1000, 2500, 5000, 10000, 20000, 30000, 40000]
    price = []
    for N in N_values:
        price.append(callfunc(15, 0.1, 0.25, 0.5, 1, 200, N))

    print(price2)
    plt.figure(figsize=(8, 6))
    plt.plot(np.array(N_values), np.array(price), marker='o', label='Numerical Price')
    plt.xlabel('Time Steps (N)')
    plt.ylabel('Option Price')
    plt.title('Option Price vs. Time Steps (N)')
    plt.grid(True)
    plt.legend()
    plt.show()
